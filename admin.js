const ADMIN_USER = 'alma';
const ADMIN_PASS = '5555';
const FALLBACK_IMAGE = "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Crect width=%22100%22 height=%22100%22 rx=%2216%22 fill=%22%23FDF5E6%22/%3E%3Ctext x=%2250%22 y=%2257%22 text-anchor=%22middle%22 font-size=%2218%22 fill=%22%238B6914%22%3EIMG%3C/text%3E%3C/svg%3E";
...
document.addEventListener('DOMContentLoaded', function () {
    if (sessionStorage.getItem('alma_admin') === 'true') {
...
function handleLogin(event) {
    ...
    if (user === ADMIN_USER && pass === ADMIN_PASS) {
        document.getElementById('loginScreen').style.display = 'none';
        document.getElementById('adminPanel').style.display = 'block';
        sessionStorage.setItem('alma_admin', 'true');
        initializeAdmin();
    } else {
...
function logout() {
    sessionStorage.removeItem('alma_admin');
...
function createEmptySize() { return { size: '', unit: 'حجم', price: '' }; }
...
function addSizeRow(sizeData) {
    ...
    row.innerHTML = '<input type="text" class="size-value" placeholder="صغير / وسط / كبير" value="' + safeSize.size + '"><select class="size-unit"><option value="حجم" selected>حجم</option></select><input type="number" class="size-price" min="0" placeholder="السعر ₪" value="' + safeSize.price + '"><button type="button" class="btn-remove-size" onclick="removeSizeRow(this)">حذف</button>';
...
function renderOrderDetails(order) {
    ...
    return '<div class="order-details"><div class="order-items-list">' + itemsHtml + '</div><div class="order-meta"><div><strong>الاسم:</strong> ' + (order.customerName || '-') + '<br><strong>الهاتف:</strong> ' + (order.customerPhone || '-') + '<br><strong>العنوان:</strong> ' + (order.address || '-') + '</div><div><strong>طريقة التوصيل:</strong> ' + (order.delivery === 'pickup' ? 'استلام من رام الله' : 'توصيل') + '<br><strong>المنطقة:</strong> ' + DELIVERY_REGION_LABEL(order.region) + '<br><strong>المجموع الفرعي:</strong> ' + formatCurrency(order.subtotal) + '<br><strong>التوصيل:</strong> ' + formatCurrency(order.deliveryCost || 0) + '<br><strong>الإجمالي:</strong> ' + formatCurrency(order.total) + '</div></div>' + (order.notes ? '<div class="order-item-card"><strong>ملاحظات:</strong><div>' + order.notes + '</div></div>' : '') + '</div>';
}
...
function renderRevenueChart() {
    ...
    charts.revenue = new Chart(document.getElementById('revenueChart'), {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'الإيراد', data: values, borderColor: '#8B6914', backgroundColor: 'rgba(139,105,20,0.15)', fill: true, tension: 0.35 }] },
...
function renderRegionChart() {
    ...
    charts.region = new Chart(document.getElementById('regionChart'), {
        ...
        data: {
            ...
            datasets: [{ data: regions.map(function (region) { return orders.filter(function (order) { return (order.region || 'pickup') === region; }).length; }), backgroundColor: ['#8B6914', '#C9A96E', '#D4A574', '#E8D5A3'] }]
        },
        options: { responsive: true, maintainAspectRatio: false }
    });
}
...
function renderTopProductsChart() {
    ...
    charts.topProducts = new Chart(document.getElementById('topProductsChart'), {
        ...
        data: {
            ...
            datasets: [{ label: 'الكمية', data: topProducts.map(function (item) { return item.qty; }), backgroundColor: '#8B6914' }]
        },
