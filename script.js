...
let cart = normalizeCartItems(JSON.parse(localStorage.getItem('alma_cart') || '[]'), normalizeProducts(DEFAULT_PRODUCTS));
let deliveryMethod = localStorage.getItem('alma_delivery_method') || 'pickup';
...
function applySettings() {
    ...
    if (tiktokLink) {
        ...
        tiktokLink.style.display = siteSettings.tiktokLink ? 'inline-flex' : 'none';
    }
}
...
function saveCart() {
    localStorage.setItem('alma_cart', JSON.stringify(normalizeCartItems(cart, products.length ? products : normalizeProducts(DEFAULT_PRODUCTS))));
}

function setDeliveryMethod(method) {
    deliveryMethod = method;
    localStorage.setItem('alma_delivery_method', method);
